						...
						:input (lambda (self scancode pressed)
							 (when (and (not pressed) (sdl2:scancode= scancode :scancode-space))
							   (if (not (-> self 'double_input))
							       (progn
								 (switch-scene "level_1.map" (lambda ()
											       (play-music "cool_nescaline.mp3")
											       ))
...
;;============ level 5 custom ===============
(defun reset-button-func (sc level)
  (let ((reset_button (find-object sc "reset_button")))
    (setf (interactible-on-interact-script reset_button)
	  (lambda (self obj)
	    ...
	    (reload-scene level (list *player* reset_button))))) )
(setf (scene-on-load *level_5_scene*) (lambda (sc)
					(reset-button-func sc "level_5.map")))
;;============ level 6 =======================
		...
		(lambda (self obj)
		  ...
		  (when (not (-> self 'interaction_occured))
		    (let ((hidden_stairs (make-tile :tile-sheet-path "tile_sheet.png"
		      ...
		      ;;play sound effect
		      (play-sound-effect "hidden_door_open.wav")
...
(setf (scene-on-load *level_8_scene*) (lambda (sc)
					(reset-button-func sc "level_8.map")))
